...
class Routine(YikObject):
    ...
    def __init__(self, parent, target: Callable[[Any], Any] = None, initialize=None, frequency: int = 1,
                 start_delay: float = 1, precise: bool = True):
        """
        ...
        :param precise: True removes tps prefire to payback owed ticks. Otherwise, use threading intervals
        """
    ...
    def _precise_loop_first_thread(self):
        amount = self.frequency / 2
        while True:
            if amount == self.frequency:
                break
            self._wrapper_target()
            self._debug_ticks_completed += 1
            amount += 1
    ...
    def _precise_loop_second_thread(self):
        amount = 0
        while True:
            if amount == self.frequency:
                break
    ...
    def _precise_loop(self):

        x = threading.Thread(target=self._precise_loop_first_thread)
        x.start()
    ...
    def start(self):
        ...
        if self.precise:
            n = threading.Thread(target=self._precise_loop)
        else:
            n = threading.Thread(target=self._unprecise_loop)
        n.start()
...
class CanTick(YikObject):
    ...
    def __init__(self, parent: _PynamicsObjTyping, precise=True, *args, **kwargs):
        self.precise = precise
    ...
    def __post_init__(self, parent: _PynamicsObjTyping, routine_include=False, routine_frequency=128, routine_target=None):
        ...
        if routine_include:
            if routine_target is None:
                self._routine = Routine(self, frequency=routine_frequency, target=self.__pn_routine_update__, precise=self.precise)
            else:
                self._routine = Routine(self, frequency=routine_frequency, target=routine_target, precise=self.precise)
        else:
